...
def test_decision_engine_real():
    ...
    result = engine.decide(0.8, 0.9, 3, 0.9, True, False, 2, 0.6, 0.0)
    assert result.status == "real"


def test_decision_engine_fake():
    engine = DecisionEngine()
    result = engine.decide(0.1, 0.2, 0, 0.2, False, False, 0, 0.0, 0.0)
    assert result.status == "fake"


def test_decision_engine_uncertain():
    engine = DecisionEngine()
    result = engine.decide(0.5, 0.5, 1, 0.5, False, False, 0, 0.2, 0.0)
